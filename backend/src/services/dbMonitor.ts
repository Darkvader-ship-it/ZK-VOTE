...
const EXPLAIN_THRESHOLD_MS =
    Number(process.env.DB_EXPLAIN_THRESHOLD_MS) || 500;
...
const alertHistory: Array<{ type: string; value: number; threshold: number; timestamp: string }> = [];
...
export function trackAlert(
    ...
    alertHistory.push({ type, value, threshold, timestamp: new Date().toISOString() });
