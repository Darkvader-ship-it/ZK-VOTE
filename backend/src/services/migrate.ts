 ...
 */
export function getAppliedMigrations(database: DatabaseType): AppliedMigration[] {
...
): Array<MigrationFile & { applied: boolean; applied_at: string | null; checksum_match: boolean | null }> {
...
function acquireLock(database: DatabaseType): boolean {
    ...
    if (existing) {
        const lockData = JSON.parse(existing.value) as { lockedAt: number; pid: number };
